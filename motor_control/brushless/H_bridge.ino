...
void setup() {
  ...
  for (int i = 0; i < NUM_PINS; i++) {
    ...
    attachInterrupt(digitalPinToInterrupt(Motor.hallPins[i]), stateChange[i],
                    CHANGE);
  }
...
void loop() {
  ...
  if (Motor.isMotorStopped()) {
    Motor.changing_state = true; // shows that our initmotor function is in the
                                 // middle of being called
    Motor.rotateMotor();
    Motor.changing_state = false;

    if (Motor.interrupting) { // if the interrupt is called within the initMotor
                              // function then call the function again
      Motor.rotateMotor();
