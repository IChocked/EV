...
Session::Session(const int POT_PIN, const int *DRV_PINS, bool paused,
                 bool drive_w_pot) {
  m_pot_pin = POT_PIN;
  ...
  if (m_com_buff_count == 1) {
    ...
    if (command == "MAGNET") {
      ...
      while (i < m_com_buff_count) {
        ...
        String pwm_str = String(m_com_buff[i + 2]) + String(m_com_buff[i + 3]) +
                         String(m_com_buff[i + 4]);
...
int Session::ramp_signal(int sig_in) {
  // currently does not ramp. Probably worth implementing to avoid unrealistic
  // load spikes
