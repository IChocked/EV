...
def update_labels():
    if FLAG_RUN:
        ...
        text_file_data.append(
            {str(datetime.datetime.now()): str(data_output_dict.values())})
...
for i in range(int(num_vars)):  # First Column
    data_label = tk.Label(
        window,
        textvariable=tk_stringvars[i],
        bg=color_bg,
        fg=color_label,
        font=font_text)
    data_label.place(
        relx=LABEL_COL1_PLACE['relx'],
        rely=LABEL_COL1_PLACE['rely'] +
        i *
        LABEL_SPACE,
        anchor='w')
# ---------------------
...
for i in range(num_magnets):
    magnet_entry = tk.Entry(
        window,
        width=2,
        font=font_magnet,
        text=magnet_tkvars[i],
        bg=color_label)
...
smv_label = tk.Label(
    window,
    text="made by Bruin SuperMileage",
    bg=color_bg,
    fg=color_fg,
    font=font_magnet)
